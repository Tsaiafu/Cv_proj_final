...
from train_face import recognize_face
from hw2_recognizer import hw2_recognizer
from resnet_recognizer import resnet_recognizer
from test_cnn_def import predict_face_class
...
if __name__ == '__main__':
    ...
    save_path = './cnn2/result/'

    def load_images(file_pathname):

        result_paths = []   # 儲存所有的圖片路徑

        # 載入圖片
        for filename in os.listdir(file_pathname):

            # print(filename)
            for picture in os.listdir(file_pathname + '/' + filename):
                # print(file_pathname + filename + '/' + picture)

                #只取檔名
                picture_na =  os.path.basename(picture).split('.')[0]
                # print(picture_na)
                # print(picture)
                ...
                if image is not None and image.shape[2] == 4:  # 檢查是否有4通道
                ...
                if image is not None:
                    ...
                    # return [save_img + filename + picture]  # 返回儲存的圖片路徑
                else:
                    pass
                    # return [save_img + filename + picture]
        #回傳照片路徑位置
        return result_paths
                #     return []
    ...
    img_list = load_images("./cnn_test_fast")
    ...
    for img_path in tqdm(img_list):
        ...
        for i in range(dets.shape[0]):
            ...
            if score > 0.9 :
                # # 在圖片上繪製矩形框   2為象素
                if ymin >= 0 and ymax <= image.shape[0] and xmin >= 0 and xmax <= image.shape[1]:
                    #cropped_image = image[int(ymin):int(ymax), int(xmin):int(xmax)]
                    cropped_image = image2[int(ymin):int(ymax), int(xmin):int(xmax)]
                # # 顯示圖片

                    #[cla,face_score] = recognize_face(cropped_image)
                    # if cla == 1 and face_score< 60 :
                    #     image2 = cv2.putText(image2, 'cage:' + str(round(face_score)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    # elif cla == 2 and face_score<60:
                    #     image2 = cv2.putText(image2, 'Trump:' + str(round(face_score)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    # else:
                    #     image2 = cv2.putText(image2, 'Stranger:' + str(round(face_score)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)


                    # class_list = os.listdir('./train_face_pict_H')
                    # [cla, face_score] = hw2_recognizer(cropped_image,class_list,'HOG')
                    # if cla == 1 :
                    #     image2 = cv2.putText(image2, 'cage:' + str(round(face_score)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    # elif cla == 2 :
                    #     image2 = cv2.putText(image2, 'Trump:' + str(round(face_score)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    ...
                    cropped_image = np.array(cv2.resize(cropped_image,(160,160)))
                    cropped_image = torch.from_numpy(cropped_image / 255.0).float()
                    cropped_image = cropped_image.permute(2, 0, 1)
                    [face_score, cla] = resnet_recognizer([cropped_image])
                    if cla == 0 and face_score > 1 :
                        image2 = cv2.putText(image2, 'IU:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    elif cla == 1 and face_score > 1:
                        image2 = cv2.putText(image2, 'Trump:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    else:
                        image2 = cv2.putText(image2, 'Stranger:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)

                    # cropped_image =Image.fromarray(cv2.cvtColor(cropped_image,cv2.COLOR_BGR2RGB))
                    # [cla,face_score] = predict_face_class(cropped_image)
                    # if cla == 0 and face_score > 1 :
                    #     image2 = cv2.putText(image2, 'cage:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    # elif cla == 1 and face_score > 1 :
                    #     image2 = cv2.putText(image2, 'Trump:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    # else:
                    #     image2 = cv2.putText(image2, 'Stranger:' + str(round(face_score,3)) , (int(xmin), int(ymin)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 2)
                    ...
                    fout.write('{} {} {} {} {} {} {}\n'.format(xmin, ymin, xmax, ymax, score, cla, face_score))
                    image2 = cv2.rectangle(image2, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (0, 255, 0), 1)
                    image2 = cv2.putText(image2, str(round(score,4)), (int(xmin), int(ymax+10)),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0), 2)
        ...
        picture_na = img_path.split('/')[-1]
        cv2.imwrite('./cnn2/result_pict/' + picture_na, image2)
            # #抓取圖片位置
